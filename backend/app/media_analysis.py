...
class MediaAnalysisManager:
    ...
    async def create_job(
        ...
        job['environment'] = environment_slug
        return job
    ...
    async def list_jobs(self, tenant_id: str, limit: int = 20, offset: int = 0) -> Dict[str, Any]:
        ...
        query = """
            SELECT maj.*, te.slug AS environment
            FROM media_analysis_jobs maj
            LEFT JOIN tenant_environments te ON te.tenant_environment_id = maj.tenant_environment_id
            WHERE maj.tenant_id = $1
        """
        args = [tenant_id]
        if environment_id:
            query += ' AND maj.tenant_environment_id = $2 ORDER BY maj.created_at DESC LIMIT $3 OFFSET $4'
            args.extend([environment_id, limit, offset])
            count_query = 'SELECT COUNT(*) AS count FROM media_analysis_jobs WHERE tenant_id = $1 AND tenant_environment_id = $2'
            count_result = await db_manager.fetch_one(count_query, tenant_id, environment_id)
        else:
            query += ' ORDER BY maj.created_at DESC LIMIT $2 OFFSET $3'
        ...
        jobs = await db_manager.fetch_all(query, *args)
    ...
    async def get_job(self, job_id: str, tenant_id: str, role: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        args = [job_id]
        if environment_id:
            query += ' AND maj.tenant_environment_id = $2'
            args.append(environment_id)
        job = await db_manager.fetch_one(query, *args)
        if not job:
            return None
    ...
    async def process_job(self, job_id: str, media_bytes: Optional[bytes] = None):
        ...
        db_manager.set_request_context(
            tenant_id=tenant_id,
            environment_id=str(job_row['tenant_environment_id']) if job_row.get('tenant_environment_id') else None,

...
class ArtifactStorageManager:
    ...
    async def store_media_artifact(
        self,
        tenant_id: str,
        job_id: str,
        filename: str,
        ...
        """Store dashboard-uploaded image/video source media in S3."""
        self._lazy_init()

        extension = os.path.splitext(filename or "")[1].lower() or ".bin"
        s3_key = f"{str(tenant_id)}/media-analysis/{str(job_id)}/source{extension}"

        if not self.s3_client:
            logger.warning("S3 not available - skipping media analysis artifact storage")
            return f"mock://{str(tenant_id)}/media-analysis/{str(job_id)}/source{extension}"

        self._put_object_with_retry(s3_key, media_data, content_type)
